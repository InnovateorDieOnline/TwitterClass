...
function getFeed() {
	...
	// success
	function(data) {
		//$('#first').append('<p>oauthToken = "'+localStorage.getItem('oauthToken') + '"; oauthVerifier = "' + localStorage.getItem('oauthVerifier') +'";</p>');
		...
		for(var i = 0; i <= 10; i++) {
			$('#content').append('<li class="list-group-item">'+tweets[i].text +'</li>');
			
			$('#content').append('<li class="list-group-item"> ReTweet Count: '+ parseFloat(tweets[i].retweet_count) +'</li><br>');
			
			//$('#first').append('<p> Tags: '+ JSON.stringify(tweets[i].hashtags) +'</p>');
			
			points = points + parseFloat(tweets[i].retweet_count);			
		}
		//alert('The total numer of retweets are: ' + j);
...
//	twitterOAuth.get('https://api.twitter.com/1.1/search/tweets.json?q=%23cicmooc&src=typd',
//			//success
//			function(data){
//				console.log(data);
//				var tweets = data.text.statuses;
//				console.log(tweets);
//				for(var i = 0; i <= 10; i++) {
//					$('#content').append('<li class="list-group-item">'+tweets[i].text +'</li>');
//				}
...
function getBadges(){	
	...
	item.setAttribute('data-bb-title','Your First Class Tweet');
	item.innerHTML = 'Thanks for sharing with the class';
	item.setAttribute('data-bb-img','images/badge.png');	     
	items.push(item);
